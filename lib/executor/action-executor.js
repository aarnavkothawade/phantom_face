...
(function() {
  ...
  class ActionExecutor {
    ...
    resolveLocalProfileValue(fieldType, fallbackValue) {
      ...
      if (key.includes('first')) return profile.first_name || (profile.name ? profile.name.split(' ')[0] : '');
      if (key.includes('last')) return profile.last_name || (profile.name ? profile.name.split(' ').slice(1).join(' ') : '');
      if (key.includes('name')) return profile.name;
      if (key.includes('phone') || key.includes('mobile') || key.includes('contact') || key.includes('tel')) return profile.phone;
      if (key.includes('aadhaar') || key.includes('uid') || key.includes('aadhar')) return profile.aadhaar;
      if (key.includes('pan')) return profile.pan;
      if (key.includes('addr') || key.includes('street') || key.includes('residence') || key.includes('flat')) return profile.address;

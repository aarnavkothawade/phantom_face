  ...
  class LocalFaceDetector {
    ...
    async scanSingleImage(img) {
      ...
      for (const f of faces) {
        const faceX = Math.round(rect.left + fit.offsetX + f.x * fit.scaleX);
        const faceY = Math.round(rect.top + fit.offsetY + f.y * fit.scaleY);
        const faceW = Math.round(f.width * fit.scaleX);
        const faceH = Math.round(f.height * fit.scaleY);
        
        const padX = Math.round(faceW * 0.20);
        const padY = Math.round(faceH * 0.25);
        
        const finalX = Math.max(rect.left, faceX - padX);
        const finalY = Math.max(rect.top, faceY - padY);
        const finalRight = Math.min(rect.left + rect.width, faceX + faceW + padX);
        const finalBottom = Math.min(rect.top + rect.height, faceY + faceH + padY);

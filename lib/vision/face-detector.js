  ...
  class LocalFaceDetector {
    ...
    async scanSingleImage(img) {
      ...
      for (const f of faces) {
        // Expand to the full photo boundaries so the entire photo card/avatar is blurred perfectly
        const computedStyle = window.getComputedStyle ? window.getComputedStyle(img) : null;
        const bRadius = computedStyle ? computedStyle.borderRadius : '10px';
